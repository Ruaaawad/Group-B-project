...
def _clean_datetime(value: object) -> str:
    ...
    try:
        ...
        message = "Date must use YYYY-MM-DD HH:MM format."
...
def normalise_record(
    ...
    Args:
        ...
        payload: Raw field values supplied by the caller or GUI form.

    Returns:
        A clean record dictionary without an ID. The manager assigns IDs after
        validation.

    Raises:
        ValidationError: If the record type is unsupported or any required
            field is invalid.
    """
    ...
    if record_type == CLIENT:
        ...
        return {
            ...
            "Address Line 1": _clean_text(
                payload.get("Address Line 1"), "Address Line 1"
            ),
            "Address Line 2": _clean_text(
                payload.get("Address Line 2"),
                "Address Line 2",
                required=False,
            ),
            "Address Line 3": _clean_text(
                payload.get("Address Line 3"),
                "Address Line 3",
                required=False,
            ),
            ...
            "Phone Number": _clean_text(
                payload.get("Phone Number"), "Phone Number"
            ),
        }
    ...
    if record_type == AIRLINE:
        return {
            "Type": AIRLINE,
            "Company Name": _clean_text(
                payload.get("Company Name"), "Company Name"
            ),
        }
